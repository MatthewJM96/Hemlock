...
// clang 3.8 has a broken experimental any implementation.
#if !defined(STX_NO_STD_ANY) && defined(__clang__) && (((__clang_major__ == 3) && (__clang_minor__ < 9)) || (__clang_major__ < 3))
#define STX_NO_STD_ANY
#endif

// If using MSVC and we don't have CXX17 available, use this any implementation to avoid getting errors reported (even though it will compile using the std library impl).
#if !defined(STX_NO_STD_ANY) && defined(_MSC_FULL_VER) && !_HAS_CXX17
#define STX_NO_STD_ANY
#endif
...
#ifndef STX_ANY_STACK_SIZE_MULT
#define STX_ANY_STACK_SIZE_MULT 4
#endif
    template<size_t StackSizeMult = STX_ANY_STACK_SIZE_MULT>
    ...
    template<typename ValueType, size_t StackSizeMult = STX_ANY_STACK_SIZE_MULT>
    ...
    template<typename T, size_t StackSizeMult = STX_ANY_STACK_SIZE_MULT>
...
{
    template<size_t StackSizeMult = STX_ANY_STACK_SIZE_MULT>
    inline void swap(STX_NAMESPACE_NAME::basic_any<StackSizeMult>& lhs, STX_NAMESPACE_NAME::basic_any<StackSizeMult>& rhs) NOEXCEPT
    {
        lhs.swap(rhs);
    }
}
